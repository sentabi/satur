...
var chess = (function() {
	...
	// DOM
	oPGNBtn, oMovesSelect, oInfoBox, oCtrlForm, oNtfArea = null, oNtfClsAll = null, bInfoBox = false, aCloseCalls = [], iNtfs = 0, rDeniedTagChrs = /(^\d)|\W/g, sAlgBoxEmpty = "...", bCtrlIsDown = false,
		...
		makeSelection: function(nSquareId, bFromSolid) {
			...
			if (fourBtsLastPc > 8) {
				...
				if (iHistPointr + 1 < aHistory.length && etc.isValidMove(nFrstFocus % 10 - 1, (nFrstFocus - nFrstFocus % 10) / 10 - 2, nSquareId % 10 - 1, (nSquareId - nSquareId % 10) / 10 - 2)) {
					if (confirm("Langkah si enggo dalankendu bene adi gerakkendu, labo dalih?")) { trimHistory(); }
					else { return; }
				}
	...
	function getInCheckPieces() {
		...
		if (bNoMoreMoves) {
			if (bCheck) {
				var sWinner = flagWhoMoved ? "Mbiring" : "Mbentar";
				oGameInfo.Result = flagWhoMoved ? "0-1" : "1-0";
				sendMsg((oGameInfo.hasOwnProperty(sWinner) ? oGameInfo[sWinner] : sWinner) + " menang.", "Raja  enggo (<em>mat<\/em>).", 10000);
				sMovesList = sMovesList.replace(/\+$/, "#");
			} else {
				oGameInfo.Result = "1/2-1/2";
				sendMsg("Remis", "sada pe lanai lit si banci erdalan (<em>remis<\/em>).", 10000);
			}
			bGameNotOver = false;
		} else if (oGameInfo.hasOwnProperty("Result") && oGameInfo.Result.search(/^(\d+\-\d+)$/) > -1 && iHistPointr === aHistory.length - 1) {
			var sWinner = oGameInfo.Result.valueOf() === "1-0" ? "Mbentar" : "Mbiring";
			sendMsg((oGameInfo.hasOwnProperty(sWinner) ? oGameInfo[sWinner] : sWinner) + " menang.", "Lawanna menyerah.", 10000);
			bGameNotOver = false;
	...
	function resetBoard() {
		...
		oMovesSelect.innerHTML = "<option>Mulai main<\/option>";
	...
	*/
	function writeHistory(bGraphRendrng, nStartPt, nEndPt, nPieceId, nTarget, nPromo) {
		...
		oMovesSelect.innerHTML = "<option>Mulai main..<\/option><option>" + sMovesList.replace(/¶/g,"<\/option><option>") + "<\/option>";
...
// DOM private APIs
	...
	function showInfo() {
		...
		var textTag = Array();
			textTag["White"] = "Mbentar";
			textTag["Black"] = "Mbiring";
			textTag["Round"] = "Ronde";
			textTag["Date"] = "Tgl.";
			textTag["Event"] = "Sesi";
			textTag["Result"] = "Si Menang";
			textTag["Site"] = "Situs";
			
		for (var iTagTxt in oGameInfo) {
			oInfoPar.appendChild(setAttribs.call(document.createElement("span"), ["className", "infoKey"], ["onclick", changeTagName], ["innerHTML", textTag[iTagTxt]]));
		...
		setAttribs.call(oNewField, ["className", "chessCtrlBtn"], ["onclick", addInfoTag], ["innerHTML", "Tambah Tag"]);
		setAttribs.call(oCloseInfo, ["className", "chessCtrlBtn"], ["onclick", hideInfo], ["innerHTML", "Tutup"]);
		oInfoPar.appendChild(document.createElement("br"));
	...
	return {
		...
		place: function(oWhere) {
			if (oBoardsBox) { oBoardsBox.parentNode.removeChild(oBoardsBox); }
				...
				setAttribs.call(oInfoBtn, ["className", "chessCtrlBtn"], ["onclick", showInfo], ["innerHTML", "Info"]);
